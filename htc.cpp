...
/**
 * Menu navigation function, accepts only integers in its input through cin
 * @param N/A
...
/**
 * Replaces the input words from a user into the symbols for AutoHotkey. i.e. "alt" becomes "!"
...
/**
 * Replaces the AutoHotkey input symbols from a file into the words for a user to understand
 * @param const string input
 * @return string output
 */
...
/**
 * Traverses an input file line by line looking for "pause=" and "exit=".
 * FIXME: Security issues with reading a file like this.
 * @param const string input
 * @return string output
 */
void ShowKeybinds(std::string filename) { // get the key values of pause and exit
	...
	while (std::getline(inFile, line)) {
		if (line.find("pause=") != std::string::npos) {
			pauseText = line.substr(line.find("pause=") + 6); // load the rest of the line after going past "pause="
		}
...
/**
 * Takes an input string and interprets it as AutoHotkey would. If it would not fire in AutoHotkey, it returns false and wipes the string to "".
 * @param string * input
 * @return bool isValidString
 */
bool ValidateHotkey(std::string * uncheckedString) {
	std::string stringToCheck = *uncheckedString; // Regex does not accept pointers
	bool isValidString = false;
	std::regex validator("^([\^!\+#<>]*(F(1[0-9]?|2[0-4])|Numpad(0|[1-9]|Dot|Enter|Div|Mult|Add|Sub)|CapsLock|NumLock|ScrollLock|Up|Down|Left|Right|Tab|Enter|Space|Esc|Backspace|Delete|Insert|Home|End|PgUp|PgDn|LButton|RButton|MButton|WheelUp|WheelDown))*$");
	if (std::regex_match(stringToCheck, validator)) {
		return isValidString = true;
	}
	else {
		*uncheckedString = ""; // If it isn't valid, delete whatever is there and replace it with something safe
...
void SetupPreferences() { // one time setup, called only if .\lib\htcprefs.ini does not exist
	...
	try {
		outfile.close(); // Closes when exiting scope, but may have an error.
	}
